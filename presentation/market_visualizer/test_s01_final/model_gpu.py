...
def simulate_market_gpu(
    ...
    # ZA系: 投資ユニバース (専門セクター全社 + グラフ隣接 + ランダム少数)
    # 銘柄あたり参加者数を N によらず一定に保ち、不均衡の粒度 sd(I_j)~sqrt(H_j) を保存する。
    if config.universe_mode == "sector_graph":
        uni_np = np.zeros((n_inv, n), dtype=bool)
        adj = true_w > 0
        expertise_np = investor_df["expertise_sector"].to_numpy()
        firm_load = np.zeros(n, dtype=int)  # 銘柄ごとの参加者数 (ハブキャップ用)
        for i in rng.permutation(n_inv):
            sector_firms = np.where(sectors == expertise_np[i])[0]
            uni_np[i, sector_firms] = True
            cand = np.zeros(n, dtype=bool)
            if sector_firms.size > 0 and adj.any():
                cand |= adj[sector_firms].any(axis=0) | adj[:, sector_firms].any(axis=1)
            if config.universe_random_extra > 0:
                extra = rng.choice(n, size=min(config.universe_random_extra, n), replace=False)
                cand[extra] = True
            cand &= ~uni_np[i]
            if config.universe_hub_cap > 0:
                # 隣接/ランダム経由の参加は、参加者数がキャップ未満の銘柄のみ
                cand &= firm_load < config.universe_hub_cap
            uni_np[i, cand] = True
            firm_load += uni_np[i]
            # 上限キャップ: セクター銘柄は保持し、それ以外を間引く
            if config.universe_max_size > 0 and uni_np[i].sum() > config.universe_max_size:
                non_sector = np.where(uni_np[i] & ~np.isin(np.arange(n), sector_firms))[0]
                n_drop = int(uni_np[i].sum() - config.universe_max_size)
                if n_drop > 0 and non_sector.size > 0:
                    drop = rng.choice(non_sector, size=min(n_drop, non_sector.size), replace=False)
                    uni_np[i, drop] = False
        universe_mask = torch.from_numpy(uni_np).to(device).float()  # (n_inv, n)
        investor_df["universe_size"] = uni_np.sum(axis=1)
    else:
        universe_mask = None
        investor_df["universe_size"] = n
    ...
    # market_visualizer 用の軽量ログ。
    # visual_log_enabled=False のときは既存挙動・返り値への影響を最小化する。
    visual_firm_records = []
    visual_investor_records = []
    visual_order_records = []
    ...
    for t in range(t_max):
        ...
        if config.rate_regime_center > 0.0:
            ...
            if rate_kappa > 0.0 and config.rate_kappa_pos_scale != 1.0:
                rate_kappa *= config.rate_kappa_pos_scale
        else:
            rate_kappa = 0.0
        ...
        # リードラグ: 過去20日の外生金利変化がスコアを遅れて圧迫 (割引率の遅効)
        if config.rate_lag_score_beta != 0.0 and len(dy_exo_hist) >= 20:
            dy_ma20 = float(np.mean(dy_exo_hist[-20:]))
            score = score - (
                config.rate_lag_score_beta * dy_ma20 * (rate_sens[:, None] / 0.10)
            )
        ...
        # ZA系: ユニバース外の銘柄は売買しない
        if universe_mask is not None:
            z_buy = z_buy * universe_mask
            z_sell = z_sell * universe_mask
        ...
        if config.visual_log_enabled:
            _vis_cash_before = cash.clone()
            _vis_holdings_before = holdings.clone()
            _vis_order_prices = firm_prices.clone()
        else:
            _vis_cash_before = None
            _vis_holdings_before = None
            _vis_order_prices = None
        ...
        if config.visual_log_enabled and (t % max(1, int(config.visual_log_stride)) == 0):
            # 上位注文だけを保存する。全注文保存はすぐ巨大化するため、MVPでは value top-k。
            order_value = buy_orders + sell_orders
            flat = order_value.flatten()
            nz = torch.nonzero(flat > 0, as_tuple=False).flatten()
            if nz.numel() > 0:
                if int(config.visual_log_top_orders) > 0 and nz.numel() > int(config.visual_log_top_orders):
                    _, local_idx = torch.topk(flat[nz], int(config.visual_log_top_orders))
                    chosen = nz[local_idx]
                else:
                    chosen = nz
                inv_idx = (chosen // n).detach().cpu().numpy()
                firm_idx = (chosen % n).detach().cpu().numpy()
                buy_np = buy_orders.detach().cpu().numpy()
                sell_np = sell_orders.detach().cpu().numpy()
                price_np = _vis_order_prices.detach().cpu().numpy()
                cb_np = _vis_cash_before.detach().cpu().numpy()
                ca_np = cash.detach().cpu().numpy()
                hb_np = _vis_holdings_before.detach().cpu().numpy()
                ha_np = holdings.detach().cpu().numpy()
                for ii, jj in zip(inv_idx, firm_idx):
                    bval = float(buy_np[ii, jj])
                    sval = float(sell_np[ii, jj])
                    if bval <= 0.0 and sval <= 0.0:
                        continue
                    side = "buy" if bval >= sval else "sell"
                    val = bval if side == "buy" else sval
                    px = float(price_np[jj])
                    visual_order_records.append({
                        "day": int(t),
                        ...
                        "side": side,
                        "value": float(val),
                        "quantity": float(val / max(px, 1e-12)),
                        "price": px,
                        "cash_before": float(cb_np[ii]),
                        "cash_after": float(ca_np[ii]),
                        "position_before": float(hb_np[ii, jj]),
                        "position_after": float(ha_np[ii, jj]),
                    })
        ...
        if config.portfolio_rebalance_rate > 0.0:
            ...
            if universe_mask is not None:
                # ユニバース内で正規化した時価総額ウェイトへ寄せる
                w_uni = current_weights.unsqueeze(0) * universe_mask  # (n_inv, n)
                w_uni = w_uni / torch.clamp(w_uni.sum(dim=1, keepdim=True), min=1e-12)
                target_hold_value = (wealth_now - target_cash).unsqueeze(1) * w_uni
            else:
        ...
        if config.generate_dgs10:
            ...
            if config.dgs10_flight_beta > 0.0:
                ...
                if config.dgs10_flight_decay > 0.0:
                    # 状態変数型: 閾値超えで点火し減衰。持続中は当日リターン全体が結合
                    if abs(sp_ret_raw) > thr:
                        flight_state = 1.0
                    else:
                        flight_state *= config.dgs10_flight_decay
                    stock_term += config.dgs10_flight_beta * flight_state * sp_ret_raw
                else:
                    # 単日超過分型 (旧)
                    excess_r = np.sign(sp_ret_raw) * max(abs(sp_ret_raw) - thr, 0.0)
                    stock_term += config.dgs10_flight_beta * excess_r
            # リードラグ: 株の20日トレンドを金利が遅れて追う (景気期待チャネル)
            if config.dgs10_stocktrend_beta > 0.0 and len(sp_ret_hist) >= 20:
                stock_term += config.dgs10_stocktrend_beta * float(np.mean(sp_ret_hist[-20:]))
            dy_new = dy_exo_part + stock_term
            ...
            sp_ret_hist.append(float(sp_ret_raw))
            dy_exo_hist.append(float(dy_exo_part))
            if len(sp_ret_hist) > 30:
                del sp_ret_hist[0]
                del dy_exo_hist[0]
        ...
        records.append({
            ...
            "sp500": sp_ret,
            "DGS10": dgs10_change,
            "_hhi": float((((cash + (holdings*firm_prices.unsqueeze(0)).sum(1)) / (cash + (holdings*firm_prices.unsqueeze(0)).sum(1)).sum())**2).sum().item()),
            "_imbal": float(imbalance.abs().mean().item()),
        })

        if config.visual_log_enabled and (t % max(1, int(config.visual_log_stride)) == 0):
            firm_limit = int(config.visual_log_firm_limit)
            firm_ids = range(n if firm_limit <= 0 else min(n, firm_limit))
            fw = weights.detach().cpu().numpy()
            fi = imbalance.detach().cpu().numpy()
            fp = firm_prices.detach().cpu().numpy()
            fr = firm_return.detach().cpu().numpy()
            bv = buy_value.detach().cpu().numpy()
            sv = sell_value.detach().cpu().numpy()
            fv = firm_vol.detach().cpu().numpy()
            xn_sig = (x * dim_w[None, :]).sum(dim=1).detach().cpu().numpy()
            mc = market_caps.detach().cpu().numpy()
            for j in firm_ids:
                visual_firm_records.append({
                    "day": int(t),
                    "Date": dates[t].strftime("%Y-%m-%d"),
                    "firm_id": int(j),
                    ...
                    "price": float(fp[j]),
                    "return": float(fr[j]),
                    "market_cap": float(mc[j]),
                    "market_weight": float(fw[j]),
                    "latent_signal": float(xn_sig[j]),
                    "imbalance": float(fi[j]),
                    "buy_value": float(bv[j]),
                    "sell_value": float(sv[j]),
                    "firm_vol": float(fv[j]),
                    "market_vol_t": float(market_vol_t),
                    "common_noise": float(common_noise),
                    "jump_abs": float(jump_abs),
                    "down_var_ewma": float(down_var_ewma),
                    "disaster_intensity": float(disaster_intensity),
                })

            inv_limit = int(config.visual_log_investor_limit)
            inv_ids = range(n_inv if inv_limit <= 0 else min(n_inv, inv_limit))
            hold_value = holdings * firm_prices.unsqueeze(0)
            wealth_now = cash + hold_value.sum(dim=1)
            daily_buy = buy_orders.sum(dim=1)
            daily_sell = sell_orders.sum(dim=1)
            cash_np = cash.detach().cpu().numpy()
            wealth_np = wealth_now.detach().cpu().numpy()
            hold_np = hold_value.sum(dim=1).detach().cpu().numpy()
            db_np = daily_buy.detach().cpu().numpy()
            ds_np = daily_sell.detach().cpu().numpy()
            for i in inv_ids:
                visual_investor_records.append({
                    "day": int(t),
                    "Date": dates[t].strftime("%Y-%m-%d"),
                    "investor_id": int(i),
                    "cash": float(cash_np[i]),
                    "holding_value": float(hold_np[i]),
                    "wealth": float(wealth_np[i]),
                    "buy_value": float(db_np[i]),
                    "sell_value": float(ds_np[i]),
                    "expertise_sector": int(investor_df.loc[i, "expertise_sector"]),
                    "vol_sensitivity": float(investor_df.loc[i, "vol_sensitivity"]),
                    "risk_tolerance": float(investor_df.loc[i, "risk_tolerance"]),
                    "graph_quality": float(investor_df.loc[i, "graph_quality"]),
                    "recognized_edges": int(investor_df.loc[i, "recognized_edges"]),
                    "universe_size": int(investor_df.loc[i, "universe_size"]),
                })
    ...
    aux = {"config": config_dict, "firm_snapshots": pd.DataFrame(firm_records)}
    if config.visual_log_enabled:
        true_edges = []
        src, dst = np.where(true_w > 0)
        for u, v in zip(src, dst):
            true_edges.append({
                "src_firm_id": int(u),
                "dst_firm_id": int(v),
                "true_weight": float(true_w[u, v]),
            })

        subjective_edges = []
        subj_np = subjective_graphs.detach().cpu().numpy()
        edge_limit = int(config.visual_log_subjective_edges_per_investor)
        for i in range(n_inv):
            ii, jj = np.where(subj_np[i] > 0)
            if ii.size == 0:
                continue
            vals = subj_np[i, ii, jj]
            order = np.argsort(-vals)
            if edge_limit > 0:
                order = order[:edge_limit]
            for pos in order:
                subjective_edges.append({
                    "investor_id": int(i),
                    "src_firm_id": int(ii[pos]),
                    "dst_firm_id": int(jj[pos]),
                    "perceived_weight": float(vals[pos]),
                })

        aux.update({
            "visual_firm_states": pd.DataFrame(visual_firm_records),
            "visual_investor_states": pd.DataFrame(visual_investor_records),
            "visual_orders": pd.DataFrame(visual_order_records),
            "true_graph_edges": pd.DataFrame(true_edges),
            "subjective_graph_edges": pd.DataFrame(subjective_edges),
        })
