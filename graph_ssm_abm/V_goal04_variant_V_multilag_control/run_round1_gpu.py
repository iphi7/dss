...
CONFIGS={
    'V00_R03_recheck': cfg(),
    # ACF長期尾を弱める：参加/出来高増幅とdown memoryを少し下げる
    'V01_less_persistence': cfg(participation_vol_power=0.70, impact_activity_scale=0.45, impact_activity_clip=1.60, down_ewma_decay=0.42, stoploss_universal_scale=0.010),
    # 高ボラ十分位leverageを弱める：非対称impact/fearをさらに弱めるがtail noiseは維持
    'V02_less_leverage': cfg(asym_pi_scale=0.80, down_ewma_decay=0.45, stoploss_universal_scale=0.005, participation_vol_power=0.80, impact_activity_scale=0.55),
    # 長期レジームだけ少し散らす：非常に弱いrebalancing
    'V03_tiny_turnover': cfg(portfolio_rebalance_rate=0.00015, portfolio_cash_target=0.05, participation_vol_power=0.80, impact_activity_scale=0.55, down_ewma_decay=0.45),
}
...
def main():
    ...
    append('## Round1: 複数lag ACF・複数日leverageを目的関数化\n\nR03を基準に、ACF曲線全体とleverage decile 1/10 の 1d〜20d、およびglobal leverage curveを評価する。')
    all_rows=[]; aggs=[]
