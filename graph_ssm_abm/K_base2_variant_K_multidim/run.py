"""
K_base2_variant_K_multidim/run.py

比較:
- no_graph: W_i を無視、各投資家は dim0(public) + dim1(sector expert) のみ使用
- with_graph: W_i 経由で dim2(private) を推定 + pred_next にもネットワーク項を追加

Usage:
    python3 graph_ssm_abm/K_base2_variant_K_multidim/run.py
"""
...
def run_variant(hist: pd.DataFrame, use_graph: bool, outdir: Path) -> pd.DataFrame:
...
def main() -> None:
    ...
    no_graph_dir   = Path("graph_ssm_abm/K_base2_variant_K_multidim/results/no_graph")
    with_graph_dir = Path("graph_ssm_abm/K_base2_variant_K_multidim/results/with_graph")

    print("=== no_graph (public + sector のみ、W_i 不使用) ===")
    gen_no_graph   = run_variant(hist, use_graph=False, outdir=no_graph_dir)
    print("=== with_graph (public + sector + W_i による private 推定) ===")
    ...
    out_csv = Path("graph_ssm_abm/K_base2_variant_K_multidim/comparison_summary.csv")
