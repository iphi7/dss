...
ROOT=Path('graph_ssm_abm/S_goal01_variant_S_tail_events')
RESULTS=ROOT/'results_gpu_round3'
MEMO=ROOT/'検証メモ.md'
SEEDS=[1,2]

def cfg(**overrides):
    # R03_more_tail をベースにする。
    params=dict(
        price_impact=0.030,
        idio_vol=0.0060,
...
CONFIGS={
    'S07_pure_tail_mild': cfg(market_tail_jump_prob=0.0010, market_tail_jump_sigma=0.045, market_tail_jump_df=2.8, market_tail_jump_clip=0.14, market_tail_jump_neg_prob=0.58),
    'S08_pure_tail_mid': cfg(market_tail_jump_prob=0.0010, market_tail_jump_sigma=0.060, market_tail_jump_df=2.5, market_tail_jump_clip=0.16, market_tail_jump_neg_prob=0.62),
    'S09_pure_tail_rare_big': cfg(market_tail_jump_prob=0.0007, market_tail_jump_sigma=0.080, market_tail_jump_df=2.3, market_tail_jump_clip=0.18, market_tail_jump_neg_prob=0.65),
}
...
def aggregate(rows,label):
    cols=['std_sp500','skew_sp500','kurt_sp500','r_acf1','abs_acf5','sq_acf5','leverage_sp500_lag1_20','abs_q099','abs_q0995','abs_q0999','abs_q999_q99_ratio','lev_dec9_5d','lev_dec10_5d','min_sp500_abs','ann_return_approx']
...
def main():
    ...
    MEMO.write_text('# S_goal01_variant_S_tail_events 検証メモ\n\n',encoding='utf-8')
    real=metric_row(hist.tail(n_days).reset_index(drop=True),'real_full_tailmatch')
    append('## Round3: pure additive tail jumps with separated RNG\n\nRound1/2ではtail jumpが既存rngを消費してベースパス自体を変えていた。tail専用RNGに分離し、R03の基礎ダイナミクスを保ったまま稀な市場ジャンプだけを加える。\n\n実データ参照:\n\n'+pd.DataFrame([real]).to_markdown(index=False))
    all_rows=[]; aggs=[]
    for label,c0 in CONFIGS.items():
        rows=[]; t0=time.time()
        append(f'### {label}\n\nparams: prob={c0.market_tail_jump_prob}, sigma={c0.market_tail_jump_sigma}, df={c0.market_tail_jump_df}, neg_prob={c0.market_tail_jump_neg_prob}')
        for seed in SEEDS:
    ...
    cols=['label','std_sp500','skew_sp500','kurt_sp500','r_acf1','abs_acf5','sq_acf5','leverage_sp500_lag1_20','abs_q099','abs_q0995','abs_q0999','abs_q999_q99_ratio','lev_dec9_5d','lev_dec10_5d','ann_return_approx']
