...
CONFIGS={
    'S04_tail_freq_mild': cfg(market_tail_jump_prob=0.0030, market_tail_jump_sigma=0.022, market_tail_jump_df=3.0, market_tail_jump_clip=0.090, market_tail_jump_neg_prob=0.58),
    'S05_tail_freq_mid': cfg(market_tail_jump_prob=0.0025, market_tail_jump_sigma=0.028, market_tail_jump_df=2.8, market_tail_jump_clip=0.110, market_tail_jump_neg_prob=0.62),
    'S06_tail_mix_neg': cfg(market_tail_jump_prob=0.0020, market_tail_jump_sigma=0.034, market_tail_jump_df=2.6, market_tail_jump_clip=0.125, market_tail_jump_neg_prob=0.70),
}
...
def main():
    ...
    append('## Round2: frequent moderate market jumps\n\nRound1ではtailは補えたが、ジャンプが少数巨大すぎて尖度が暴れた。頻度を上げてサイズを下げた中ジャンプで、99.9%分位を補いつつ尖度の過剰を抑える。\n\n実データ参照:\n\n'+pd.DataFrame([real]).to_markdown(index=False))
    all_rows=[]; aggs=[]
