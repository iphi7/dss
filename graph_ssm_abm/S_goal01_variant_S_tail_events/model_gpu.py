...
def simulate_market_gpu(
    ...
    # tail jump専用RNG。既存の状態遷移・投資家行動の乱数列を変えず、
    # R03ベースに純粋にジャンプだけを重ねるために分離する。
    tail_rng = np.random.default_rng(config.seed + 1_234_567)
    n = config.n_firms
    t_max = config.n_days
    n_pub  = config.n_pub
    n_sec  = config.n_sec
    ...
    for t in range(t_max):
        ...
        # iidに近い稀な市場ジャンプ。
        # down_ewmaやactivityには過度に持続させず、分布最上位tail/尖度だけを補う狙い。
        market_tail_jump = 0.0
        if config.market_tail_jump_prob > 0.0 and tail_rng.random() < config.market_tail_jump_prob:
            mag = abs(tail_rng.standard_t(df=config.market_tail_jump_df)) * config.market_tail_jump_sigma
            sign = -1.0 if tail_rng.random() < config.market_tail_jump_neg_prob else 1.0
            market_tail_jump = float(np.clip(sign * mag, -config.market_tail_jump_clip, config.market_tail_jump_clip))
        ...
        firm_return = config.price_impact * impact_factor * imbalance + common_noise + market_tail_jump + noise + fundamental_return + market_anchor_return
