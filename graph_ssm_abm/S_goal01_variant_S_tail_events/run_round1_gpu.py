...
CONFIGS={
    'S01_tail_small': cfg(market_tail_jump_prob=0.0007, market_tail_jump_sigma=0.055, market_tail_jump_df=2.5, market_tail_jump_clip=0.16, market_tail_jump_neg_prob=0.55),
    'S02_tail_medium': cfg(market_tail_jump_prob=0.0011, market_tail_jump_sigma=0.065, market_tail_jump_df=2.3, market_tail_jump_clip=0.18, market_tail_jump_neg_prob=0.60),
    'S03_tail_neg': cfg(market_tail_jump_prob=0.0010, market_tail_jump_sigma=0.075, market_tail_jump_df=2.2, market_tail_jump_clip=0.18, market_tail_jump_neg_prob=0.72),
}
...
def main():
    ...
    append('## Round1: R03 + iid rare market tail jumps\n\nR03ではraw ACFは改善したが、絶対リターンの最上位分位と全期間尖度が不足。状態を持たない稀な市場ジャンプを追加し、ACFを増やさずtailだけを補う。\n\n実データ参照:\n\n'+pd.DataFrame([real]).to_markdown(index=False))
    all_rows=[]; aggs=[]
