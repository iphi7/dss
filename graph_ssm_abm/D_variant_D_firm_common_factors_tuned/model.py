...
class Config:
    ...
    firm_common_factors: bool = False
    n_common_factors: int = 5
    factor_phi: float = 0.92
    factor_sigma: float = 0.018
    factor_rare_prob: float = 0.012
    factor_rare_sigma: float = 0.060
    factor_market_loading: float = 0.18
    factor_sector_loading: float = 0.22
    factor_random_loading: float = 0.04
...
def simulate_market(
    ...
    factor_state = np.zeros(config.n_common_factors, dtype=float)
    factor_loadings = np.zeros((n, config.n_common_factors), dtype=float)
    if config.firm_common_factors:
        factor_loadings[:, 0] = config.factor_market_loading * rng.lognormal(mean=0.0, sigma=0.10, size=n)
        for f in range(1, config.n_common_factors):
            sector_id = (f - 1) % config.n_sectors
            sector_mask = sectors == sector_id
            factor_loadings[sector_mask, f] += config.factor_sector_loading * rng.lognormal(mean=0.0, sigma=0.15, size=sector_mask.sum())
            factor_loadings[:, f] += rng.normal(0.0, config.factor_random_loading, size=n)
        factor_loadings /= np.sqrt(max(config.n_common_factors, 1))
    ...
    for t in range(t_max):
        ...
        firm_common_eta = np.zeros(n, dtype=float)
        if config.firm_common_factors:
            factor_shock = rng.normal(0.0, config.factor_sigma, size=config.n_common_factors)
            factor_rare = rng.random(config.n_common_factors) < config.factor_rare_prob
            factor_shock += factor_rare * rng.standard_t(df=3, size=config.n_common_factors) * config.factor_rare_sigma
            factor_state = config.factor_phi * factor_state + factor_shock
            firm_common_eta = factor_loadings @ factor_state

        x = config.true_phi * x + config.true_rho * (true_w @ x) + firm_common_eta + eta
                ...
                firm_records.append(
                    {
                        ...
                        "firm_common_eta": float(firm_common_eta[j]),
