...
def simulate_market_gpu(
    ...
    for t in range(t_max):
        ...
        if config.investor_stress_scale > 0.0 and investor_stress_state > config.investor_stress_threshold:
            stress_excess = min(
            ...
            participation_factor = torch.clamp(participation_factor * pref_mult * averse_mult, 0.05, 8.0)
        else:
            stress_excess = 0.0
        ...
        if config.investor_stress_scale > 0.0 and stress_excess > 0.0 and config.risk_pref_buy_tilt != 0.0:
            pref = torch.clamp(vol_sens_t, min=0.0)
            buy_tilt = torch.clamp(1.0 + config.risk_pref_buy_tilt * stress_excess * pref, 0.2, 5.0)
            z_buy = z_buy * buy_tilt[:, None]
        if config.investor_stress_scale > 0.0 and stress_excess > 0.0 and config.risk_pref_sell_tilt != 0.0:
            pref = torch.clamp(vol_sens_t, min=0.0)
        ...
        if config.investor_stress_scale > 0.0 and stress_excess > 0.0 and config.risk_pref_size_scale > 0.0:
            pref = torch.clamp(vol_sens_t, min=0.0)
