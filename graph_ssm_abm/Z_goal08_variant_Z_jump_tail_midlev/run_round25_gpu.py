"""
Z_goal08 Round25: レジーム相関の対称化 + トレンド項

Round24 の結果:
  - スコア経由 (Z95) が優位: era1=-0.42/era2=+0.29/era3=+0.27、leverage・abs維持
  - ズレ1: era1過剰/era2弱 — 高金利期はtanhが-1に飽和、低金利期は+0.6止まり
    → center 4.5→5.5 で対称化
  - ズレ2: era3 (2020s) が+0.27 (実≈0) — 金利水準は低いが2022-23の急上昇 (インフレ)
    で割引率チャネルが復活 → rate_trend_scale: 1年前比上昇分を実効水準に加算

  Z97: Z95 + center5.5
  Z98: Z97 + trend_scale=1.0
  Z99: Z98 + beta 1.0→1.2
  Z100: Z98 + trend_scale=1.5
"""
...
RESULTS = ROOT / 'results_gpu_round25'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
CONFIGS = {
    'Z97_center55': z92_base(rate_change_score_beta=1.0, rate_regime_center=5.5),
    'Z98_trend10': z92_base(
        rate_change_score_beta=1.0, rate_regime_center=5.5, rate_trend_scale=1.0,
    ),
    'Z99_trend10_b12': z92_base(
        rate_change_score_beta=1.2, rate_regime_center=5.5, rate_trend_scale=1.0,
    ),
    'Z100_trend15': z92_base(
        rate_change_score_beta=1.0, rate_regime_center=5.5, rate_trend_scale=1.5,
    ),
}
...
def main():
    ...
    tgt['rc_rmse'] = 0.0
    append(
        '## Round25: レジーム相関の対称化+トレンド項\n\n'
        'スコア経由が優位。era1過剰/era2弱→center5.5、era3過剰→金利上昇トレンド補正。\n'
        'Z97=center5.5、Z98=+trend1.0、Z99=+beta1.2、Z100=+trend1.5。'
    )
    all_rows = []
    aggs = []
    ...
    aggdf.to_csv(ROOT / 'comparison_round25.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round25.csv', index=False)
