"""
Z_goal08 Round23 (最終): abs1≤0.30 と leverage 維持の両立点

Round22 のトレードオフ前線:
  - Z88 (iid増): abs1=0.284 ✓ だが dec9=-0.080 弱化、kurt=31
  - Z87 (pnoise0.5): abs1=0.314 未達だが dec9=-0.117、kurt=22.7 と他良好

中間 + asym増強で両立を狙う:
  Z90: Z87 + exogσ0.0061/pi0.020 + asym1.45
  Z91: 同 + asym1.6
  Z92: Z88 + asym1.6 + disasterσ0.040 (kurt制御)
"""
...
RESULTS = ROOT / 'results_gpu_round23'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
CONFIGS = {
    'Z90_mid_asym145': z86_base(
        participation_noise_sigma=0.50,
        exog_common_sigma=0.0061, price_impact=0.020,
        asym_pi_scale=1.45,
    ),
    'Z91_mid_asym16': z86_base(
        participation_noise_sigma=0.50,
        exog_common_sigma=0.0061, price_impact=0.020,
        asym_pi_scale=1.6,
    ),
    'Z92_iid_asym16': z86_base(
...
def main():
    ...
    aggdf.to_csv(ROOT / 'comparison_round23.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round23.csv', index=False)
