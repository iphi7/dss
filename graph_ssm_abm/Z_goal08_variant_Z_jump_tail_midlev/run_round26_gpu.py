"""
Z_goal08 Round26 (最終): era3 (2020s) のゼロ化

Round25 の結果:
  - トレンド項有効: era3 +0.38 (なし) → +0.26 (1.0) → +0.15 (1.5)、単調改善
  - era2=+0.379 (実+0.352) ほぼ一致、q05/q95 も一致、leverage/abs 維持
  - era1=-0.375 (実-0.316) 微過剰 — トレンド増強は era1 (60s-70s上昇期) も負に押すので注意

  Z101: Z100 + trend 2.0
  Z102: Z100 + trend 2.0 + width 1.2 (遷移シャープ化)
  Z103: Z100 + trend 2.5 + beta 0.9 (全体を薄めつつ era3 を切る)
"""
...
RESULTS = ROOT / 'results_gpu_round26'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
CONFIGS = {
    'Z101_trend20': z92_base(
        rate_change_score_beta=1.0, rate_regime_center=5.5, rate_trend_scale=2.0,
    ),
    'Z102_trend20_w12': z92_base(
        rate_change_score_beta=1.0, rate_regime_center=5.5, rate_trend_scale=2.0,
        rate_regime_width=1.2,
    ),
    'Z103_trend25_b09': z92_base(
...
def main():
    ...
    append(
        '## Round26 (最終): era3ゼロ化\n\n'
        'トレンド項が単調に有効 (era3 +0.38→+0.15)。さらに増強してゼロへ。\n'
        'Z101=trend2.0、Z102=+width1.2、Z103=trend2.5+beta0.9。'
    )
    all_rows = []
    aggs = []
    ...
    aggdf.to_csv(ROOT / 'comparison_round26.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round26.csv', index=False)
