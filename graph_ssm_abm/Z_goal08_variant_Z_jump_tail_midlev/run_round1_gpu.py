"""
Z_goal08 Round1: Y14/Y15中間 + ジャンプサイズ + 中位分位leverage の機構別検証

課題 (Y系からの引き継ぎ + ユーザー指摘):
  1. Y14/Y15の中間: ボラクラ (abs/sq ACF) を Y14 程度に保ちつつ短期 decile10 を抑える
  2. Y15 は分位5-9の leverage が薄まりすぎ (実データ 5d: -0.07〜-0.13 に対しほぼ 0)
  3. ジャンプサイズ不足: Y15 の max|r|=0.0800 は firm_return_clip=0.08 が束縛
     (実データ max=0.205, q999=0.070)

Round1 は機構を1つずつ足して効果を分離する:
  Z01: Y14/Y15 単純中間 (コントロール、firm_return_clip=0.08 のまま)
  Z02: Z01 + firm_return_clip=0.20 / exog_common_clip=0.20 (クリップ解放)
  Z03: Z02 + jump_df=2.3 (ジャンプ分布の裾を重く)
  Z04: Z02 + 余震機構 (ジャンプ後数日のボラ増幅)
  Z05: Z02 + asym_pi 閾値低下+飽和 (中位分位 leverage)
  Z06: Z02 + 投資家別ストレス記憶長 (20d leverage)
  Z07: Z02 + 余震 + asym_pi拡張 (複合)
"""
...
CONFIGS = {
    # コントロール: 単純中間。クリップは Y のまま (0.08)
    'Z01_mid_interp': z01_base(),
    # クリップ解放: firm_return_clip が jump tail を切っていた問題への対応
    'Z02_unclip': z01_base(
        firm_return_clip=0.20, exog_common_clip=0.20,
    ),
    # ジャンプ分布の裾を重く (df 3→2.3)。同頻度でサイズだけ大きく
    'Z03_heavy_df': z01_base(
        firm_return_clip=0.20, exog_common_clip=0.20,
        jump_df=2.3, exog_common_jump_sigma=0.085,
    ),
    # 余震: ジャンプ後数日のボラ増幅 → 尖度とACFを同時に補う
    'Z04_aftershock': z01_base(
        firm_return_clip=0.20, exog_common_clip=0.20,
        jump_aftershock_scale=20.0, jump_aftershock_decay=0.55,
    ),
    # 中位分位 leverage: asym_pi の閾値を下げ + 飽和で dec10 過剰を抑制
    'Z05_midlev': z01_base(
        firm_return_clip=0.20, exog_common_clip=0.20,
        asym_pi_scale=0.55, asym_pi_threshold=0.55, asym_pi_sat=1.3,
    ),
    # 投資家別ストレス記憶長: 20d leverage の持続源
    'Z06_hetero_stress': z01_base(
        firm_return_clip=0.20, exog_common_clip=0.20,
        investor_stress_decay_min=0.90, investor_stress_decay_max=0.988,
    ),
    # 複合: 余震 + 中位分位 leverage
    'Z07_aftershock_midlev': z01_base(
        firm_return_clip=0.20, exog_common_clip=0.20,
        jump_aftershock_scale=20.0, jump_aftershock_decay=0.55,
        asym_pi_scale=0.55, asym_pi_threshold=0.55, asym_pi_sat=1.3,
    ),
...
def main():
    ...
    append(
        ...
        '課題: (1) Y14/Y15中間のボラクラ、(2) Y15の分位5-9 leverage 消失、'
        '(3) ジャンプサイズ不足 (Y15のmax|r|=0.080はfirm_return_clip=0.08が束縛、実データmax=0.205)。\n'
        'Z01=単純中間(コントロール)、Z02=クリップ解放、Z03=jump_df重尾、Z04=余震、'
        'Z05=asym_pi閾値低下+飽和、Z06=投資家別ストレス記憶長、Z07=余震+midlev複合。'
    )
    all_rows = []
    aggs = []
