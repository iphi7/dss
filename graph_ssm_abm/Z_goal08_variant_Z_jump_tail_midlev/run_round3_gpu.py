"""
Z_goal08 Round3: レジーム遷移の頻度制御

Round2 の診断:
  - 系は「死」(rolling std≈0.005) と「高温」(≈0.019) の双安定
  - 実データも 0.0054〜0.0215 のレジームを持つ → 問題は双安定ではなく遷移が起きないこと
    (seed1/3 は一度も発火せず、seed2 は12年目に発火後冷めない)
  - 実データは60年で両方向に複数回遷移する

Round3 は遷移レートを制御する:
  Z13: Z08 + realized_vol_lambda=0.95 (ボラ推定を速く → 発火/消火両方を起きやすく)
  Z14: Z08 + vol_factor_max=2.2/participation_cap=2.8 (高温状態を準安定化)
  Z15: Z13+Z14 複合
  Z16: Z15 + 余震強化 (scale=30, decay=0.65) — 発火の種を増やす
  Z17: Z15 の弱め版 (vfmax=3.0, cap=3.5, lambda=0.955)
"""
...
CONFIGS = {
    # ボラ推定を速く: 発火も消火も起きやすくする
    'Z13_fastvol': z07_combo(market_vol=0.0045, realized_vol_lambda=0.95),
    # 高温状態の自己維持ゲインを削る
    'Z14_lowcap': z07_combo(
        market_vol=0.0045, vol_factor_max=2.2, participation_cap=2.8,
    ),
    # 複合
    'Z15_fastvol_lowcap': z07_combo(
        market_vol=0.0045, realized_vol_lambda=0.95,
        vol_factor_max=2.2, participation_cap=2.8,
    ),
    # 複合 + 余震強化 (発火の種)
    'Z16_z15_aftershock': z07_combo(
        market_vol=0.0045, realized_vol_lambda=0.95,
        vol_factor_max=2.2, participation_cap=2.8,
        jump_aftershock_scale=30.0, jump_aftershock_decay=0.65,
    ),
    # 弱め版
    'Z17_mild': z07_combo(
        market_vol=0.0045, realized_vol_lambda=0.955,
        vol_factor_max=3.0, participation_cap=3.5,
    ),
}
...
def main():
    ...
    append(
        '## Round3: レジーム遷移の頻度制御\n\n'
        'Round2診断: 系は死(std 0.005)と高温(0.019)の双安定で、問題は遷移が起きないこと。'
        '実データ(rolling std 0.0054-0.0215)も似たレジームを持つが両方向に複数回遷移する。\n'
        'Z13=fastvol(lambda 0.95)、Z14=lowcap(vfmax2.2/cap2.8)、Z15=複合、'
        'Z16=Z15+余震強化、Z17=弱め版。'
    )
    all_rows = []
    aggs = []
