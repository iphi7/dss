"""
Z_goal08 Round12: 相対デッドバンド + エピソードの尖度調整

Round11 の結果:
  - 災害エピソード(Z45)は構造的に成功: sq_acf5=0.298 (seed2, 目標0.228)、
    dec10 5d/10d=-0.200/-0.172 (目標-0.209/-0.176)
  - 絶対デッドバンド(Z44)は失敗: 静穏レジームでは日次|r|≈0.005 なので
    deadband=0.005 が発火の燃料 (通常サイズの下落) まで遮断し、トラップ再発
  - エピソード版は尖度が低い (9.9-12.7 vs 21.8)、max も 0.11-0.17

Round12:
  - デッドバンドを相対形に: 閾値 = down_deadband_rel × 実現ボラ (レジーム適応)
  - エピソードの日次ショックを重く (df 4→3) して単日尖度を回復

  Z48: Z45 + rel deadband 0.5
  Z49: Z45 + rel deadband 0.8
  Z50: Z45 + disaster (σ=0.055, df=3.0, decay=0.68) — 尖度/tail回復
  Z51: Z50 + rel deadband 0.5 — 統合
"""
...
RESULTS = ROOT / 'results_gpu_round12'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
def z45_base(**overrides):
    """Z43 の特大単日ジャンプを災害エピソードへ置換した構造 (Round11 Z45)。"""
    params = dict(
        ...
        disaster_prob=0.0006, disaster_sigma=0.045,
        disaster_df=4.0, disaster_decay=0.62, disaster_end=0.10,
...
CONFIGS = {
    'Z48_reldb05': z45_base(down_deadband_rel=0.5),
    'Z49_reldb08': z45_base(down_deadband_rel=0.8),
    'Z50_heavy_episode': z45_base(
        disaster_sigma=0.055, disaster_df=3.0, disaster_decay=0.68,
    ),
    'Z51_heavy_reldb05': z45_base(
        disaster_sigma=0.055, disaster_df=3.0, disaster_decay=0.68,
        down_deadband_rel=0.5,
    ),
}
...
def main():
    ...
    append(
        ...
        'Z45(エピソード)成功: sq_acf5=0.298。絶対deadbandは発火を殺すため相対形へ。'
        'エピソードのdf 4→3で単日尖度回復も試す。\n'
    ...
    aggdf.to_csv(ROOT / 'comparison_round12.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round12.csv', index=False)
