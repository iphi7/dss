"""
Z_goal08 Round15: ジャンプ2階層のくっきり分離 + sq_acf の lag プロファイル改善

ユーザー指摘 (Z56への評価):
  1. ジャンプは「超特大 ~0.2 が期間内1回」と「そこそこ 0.075~0.1」にくっきり
     分かれるべき (実データ)
  2. sq_acf は lag5 は良い (0.268) が、それ以外が実データの半分
     (lag1: 0.128 vs 0.230、lag2: 0.108 vs 0.273、lag10: 0.062 vs 0.136)
  3. leverage は概ね自然 → 壊さない

対応:
  - メガクラッシュ層 (mega_crash_*): p=1/60年、単日 -0.20×U(0.9,1.1)、負符号固定。
    事後は余震へ (1987後の+5/+9%日 → sq lag1-3 にも寄与)
  - エピソード再形状: df 3→8 (日次サイズが揃う)、σ 0.055→0.075 (初日~0.08)、
    decay 0.68→0.80・end 0.30 (持続~6日) → 連続する大きい日で sq lag1-10 を押し上げ
  - 第2余震 (2〜3週間スケール) で sq lag10-20
  - クリップ 0.20→0.24 (メガクラッシュを切らない)

  Z59: 階層分離のみ
  Z60: Z59 + 遅い第2余震 (5, 0.96)
  Z61: Z59 + 速い第1余震強化 (40, 0.60)
  Z62: Z59 + 両方
"""
...
RESULTS = ROOT / 'results_gpu_round15'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
CONFIGS = {
    ...
    'Z61_fast1': z59_base(
        jump_aftershock_scale=40.0, jump_aftershock_decay=0.60,
    ),
    'Z62_both': z59_base(
        jump_aftershock_scale=40.0, jump_aftershock_decay=0.60,
        jump_aftershock2_scale=5.0, jump_aftershock2_decay=0.96,
...
def main():
    ...
    append(
        ...
        '指摘: 超特大~0.2×1回 + そこそこ0.075~0.1 にくっきり分離すべき。'
        'sq_acfはlag5以外が実データの半分。\n'
        'Z59=メガクラッシュ+エピソード再形状(df8,σ0.075,decay0.80)、'
        'Z60=+遅い第2余震、Z61=+速い第1余震強化、Z62=両方。'
    )
    all_rows = []
    aggs = []
    ...
    aggdf.to_csv(ROOT / 'comparison_round15.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round15.csv', index=False)
