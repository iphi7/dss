"""
Z_goal08 Round16: メガクラッシュのエピソード誘発 (sq_acf希釈の解消)

Round15 の結果:
  - 2階層分離は成功: kurt=19.5-20.9 (目標21.8)、max=0.221、q999=0.078
  - leverage も回復 (dec10 5d=-0.178)
  - しかし sq_acf が全lagで低下 (sq5: 0.174→0.075)。メガクラッシュが孤立iid点として
    r^2 分散を支配するため (実1987は翌日以降±5-9%が続くが、現余震はメガ後±2.5%と弱い)

対応: mega_triggers_episode — メガクラッシュが災害エピソードを誘発し、
     暴落後の数日乱高下 (実1987: +5.3/-8.3/+9.1%) を再現。

  Z63: Z60 + メガ誘発 (intensity 1.2)
  Z64: Z63 + 第1余震強化 (scale 60)
  Z65: Z63 + エピソード頻度増 (p 0.0004→0.0006, σ 0.075→0.065)
  Z66: Z63 + エピソード持続化 (decay 0.85)
"""
...
RESULTS = ROOT / 'results_gpu_round16'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
CONFIGS = {
    'Z63_megatrig': z63_base(),
    'Z64_after60': z63_base(jump_aftershock_scale=60.0),
    'Z65_morecrisis': z63_base(disaster_prob=0.0006, disaster_sigma=0.065),
    'Z66_longcrisis': z63_base(disaster_decay=0.85),
}
...
def main():
    ...
    append(
        ...
        'Round15で2階層分離成功(kurt=20,max=0.221)がsq_acf低下(孤立megaがr2を支配)。'
        'megaがエピソードを誘発し1987型の暴落後乱高下を作る。\n'
    ...
    aggdf.to_csv(ROOT / 'comparison_round16.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round16.csv', index=False)
