"""
Z_goal08 Round17: abs/sq の再配分 (滑らかな持続を削り、危機クラスタを伸ばす)

Round16 の結果:
  - メガ誘発成功: Z64 で sq1=0.216/sq2=0.231 (実0.230/0.273) と短lagは実データ水準
  - 残る形状問題: sq5/10 が弱い (0.09 vs 実0.23/0.14)、abs1 が過剰 (0.40 vs 実0.26)
  - 実データは sq が lag5 まで平坦。モデルはエピソード幾何減衰とともに落ちる
  - Z66 (decay0.85) は sq5/10 を上げるが abs も上げてしまう

仮説: abs過剰は平常時の滑らかなボラ持続 (活動チャネル・ストレス記憶) から、
     sq不足は危機クラスタの持続不足から。両者は別チャネルなので再配分できる。

  Z67: Z64 + decay0.85 (Z64とZ66の合成)
  Z68: Z67 + impact_activity_scale 1.40→0.90 (活動由来のabs持続を削る)
  Z69: Z67 + investor_stress_decay_max 0.988→0.960 (ストレス記憶由来を削る)
  Z70: Z68 + down_ewma_decay 0.75→0.65 (下落記憶も短縮)
"""
...
RESULTS = ROOT / 'results_gpu_round17'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
CONFIGS = {
    'Z67_merge': z67_base(),
    'Z68_lowact': z67_base(impact_activity_scale=0.90),
    'Z69_shortstress': z67_base(investor_stress_decay_max=0.960),
    'Z70_lowact_shortdown': z67_base(impact_activity_scale=0.90, down_ewma_decay=0.65),
}
...
def main():
    ...
    append(
        '## Round17: abs/sqの再配分\n\n'
        'Z64でsq1/2は実データ水準。sq5/10不足とabs1過剰が残る。'
        '滑らかな持続(活動・ストレス)を削り危機クラスタを伸ばす再配分。\n'
    ...
    aggdf.to_csv(ROOT / 'comparison_round17.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round17.csv', index=False)
