"""
Z_goal08 Round9: 2層ジャンプの規模調整と冷却

Round8 の結果:
  - sq_acf 回復 (Z35 seed1: 0.252、目標0.228)。2層ジャンプ×余震は有効
  - 副作用: std膨張(0.014-0.020)、q999過剰(0.12-0.15)、dec10希釈(iidジャンプが
    大きい日を支配すると内生 leverage 相関が薄まる)
  - Z36 (中規模多め・小さめ) が最良バランス

Round9 は Z36 軸で規模を縮めて整える:
  Z37: Z36 + jump2_sigma=0.020, 特大σ=0.12, price_impact=0.025 (冷却)
  Z38: Z37 + jump_vol_coupling=0.35 (高温期の増幅を弱める)
  Z39: Z38 + 余震30/0.72 (sq_acf押し上げ)
  Z40: Z38 + price_impact=0.022 (さらに冷却)
"""
...
RESULTS = ROOT / 'results_gpu_round9'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
def z37_base(**overrides):
    """Z36 (2層ジャンプ+asym再形状) の規模縮小版。"""
    params = dict(
        ...
        exog_common_jump_prob=0.0004, exog_common_jump_sigma=0.12, jump_df=2.5,
...
CONFIGS = {
    'Z37_scaled': z37_base(),
    'Z38_vc035': z37_base(jump_vol_coupling=0.35),
    'Z39_vc035_after30': z37_base(
        jump_vol_coupling=0.35,
        jump_aftershock_scale=30.0, jump_aftershock_decay=0.72,
    ),
    'Z40_vc035_cool': z37_base(jump_vol_coupling=0.35, price_impact=0.022),
}
...
def main():
    ...
    append(
        ...
        'Round8でsq_acf回復(Z35 seed1=0.252)したがstd/q999過剰・dec10希釈。'
        'Z36軸で jump2_sigma 0.030→0.020、特大σ0.15→0.12、pi冷却。\n'
    ...
    aggdf.to_csv(ROOT / 'comparison_round9.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round9.csv', index=False)
