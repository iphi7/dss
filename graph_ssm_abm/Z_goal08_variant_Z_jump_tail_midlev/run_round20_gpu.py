"""
Z_goal08 Round20: abs_acf1 の削減 (0.40 → 0.30以下)

ユーザー指摘: |return| の acf が 0.4 超えは問題。0.3 かそれ以下に。
制約: leverage (dec10 5d/10d ≈ -0.205/-0.184) と sq_acf は壊さない。

abs_acf1 過剰の原因候補 (3系統):
  (a) 実現ボラ EWMA (λ=0.95) の平滑化が強く、参加率が滑らかに持続
  (b) ボラ→参加→ボラのループ自体の持続
  (c) リターンに占める iid 成分が少なく、持続成分 (取引由来) が支配

  Z79: Z77 + realized_vol_lambda 0.95→0.88 (aの検証)
  Z80: Z77 + participation_noise_sigma=0.35 (bの希釈)
  Z81: Z77 + exog_common_sigma 0.00445→0.0058, pi 0.025→0.021 (cの再配分)
  Z82: Z79 + Z80 の複合
"""
...
RESULTS = ROOT / 'results_gpu_round20'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
CONFIGS = {
    'Z79_fastvol088': z77_base(realized_vol_lambda=0.88),
    'Z80_pnoise035': z77_base(participation_noise_sigma=0.35),
    'Z81_noiseshift': z77_base(exog_common_sigma=0.0058, price_impact=0.021),
    'Z82_fast_pnoise': z77_base(realized_vol_lambda=0.88, participation_noise_sigma=0.35),
}
...
def main():
    ...
    aggdf.to_csv(ROOT / 'comparison_round20.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round20.csv', index=False)
