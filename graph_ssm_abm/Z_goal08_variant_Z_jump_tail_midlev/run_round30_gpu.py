"""
Z_goal08 Round30: 生リターン lag1 自己相関の修正 (ユーザー指摘)

Z113 の r_acf1 = 0.075-0.126 (実データ -0.015) と過剰に正。

機序: 株→金利→翌日スコア→株 の1日往復ループがモメンタムを合成
  r_t ↑ → Δy_t に β_sr×κ×r_t (同日) → 翌日 score に β_B×κ×Δy_t → r_{t+1} ↑
  (κ² > 0 なのでレジームによらず常に正方向)

修正: rate_score_use_exo — スコアは Δy の外生成分 (株由来項を除く) にのみ反応。
  「昨日の自分たちの売買が動かした金利には二重反応しない」という自然な仮定。
  同日相関 (株→金利チャネル) はそのまま保たれる。

  Z116: Z113 + use_exo
  Z117: Z115 (scoreβ0.45) + use_exo
  Z118: Z116 + disaster_mu -0.012→-0.008 (危機ドリフト由来の残り分の確認)
"""
...
RESULTS = ROOT / 'results_gpu_round30'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
def z111_base(**overrides):
    """Round28 最良 Z111。"""
    params = dict(rate_change_score_beta=0.7, dgs10_stock_beta=2.5)
    params.update(overrides)
    return z108_base(**params)


def z113_base(**overrides):
    """Round29 最終候補 Z113。"""
    params = dict(
        rate_change_score_beta=0.55, dgs10_df=4.5, dgs10_vol_lambda=0.95,
    )
    params.update(overrides)
    return z111_base(**params)


CONFIGS = {
    'Z116_exo': z113_base(rate_score_use_exo=True),
    'Z117_exo_sb045': z113_base(rate_score_use_exo=True, rate_change_score_beta=0.45),
    'Z118_exo_mu008': z113_base(rate_score_use_exo=True, disaster_mu=-0.008),
}
...
def main():
    ...
    tgt['rc_rmse'] = 0.0
    append(
        '## Round30: 生リターンlag1自己相関の修正\n\n'
        'r_acf1=0.08-0.13 (実-0.015)。株→金利→翌日スコア→株のループがモメンタム合成。'
        'スコアをΔyの外生成分に限定してループを切る。\n'
        'Z116=use_exo、Z117=+scoreβ0.45、Z118=+mu-0.008。'
    )
    all_rows = []
    aggs = []
    ...
    aggdf.to_csv(ROOT / 'comparison_round30.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round30.csv', index=False)
