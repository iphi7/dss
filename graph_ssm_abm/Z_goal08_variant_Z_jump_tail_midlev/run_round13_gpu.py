"""
Z_goal08 Round13: 低分位正相関の原因切り分け診断

Round12 の確定事項:
  - デッドバンド (絶対/相対とも) は発火機構と両立しない → 非推奨として確定
  - 重エピソード (Z50) は seed2/3 で sq_acf 0.26/0.20 と有効

低分位 (dec3-4) 正相関の再診断:
  正相関 = 「小さい正リターンの後にボラが上がる」なので、原因は下落記憶ではなく
  上昇側のチャネルにあるはず。候補:
  (a) 公開観測の momentum 項: 正リターン → 観測強気 → 買い活動増 → 取引量 → インパクト増幅
  (b) 市場アンカー: 価格 > アンカー → 引き戻し圧 → ボラ増
  (c) 全員一律 fear (stoploss_universal 残り 0.0005)

  Z52: Z50 + obs_momentum_scale=0 (aの検証)
  Z53: Z50 + market_anchor_strength=0 (bの検証)
  Z54: Z50 + stoploss_universal_scale=0 (cの検証)
  Z55: Z50 + risk_averse_withdraw 0.29→0.60 (微小下落後の退避を強め相対的に相殺)
"""
...
RESULTS = ROOT / 'results_gpu_round13'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
CONFIGS = {
    'Z52_nomom': z50_base(obs_momentum_scale=0.0),
    'Z53_noanchor': z50_base(market_anchor_strength=0.0),
    'Z54_nofear': z50_base(stoploss_universal_scale=0.0),
    'Z55_withdraw06': z50_base(risk_averse_withdraw_scale=0.60),
}
...
def main():
    ...
    append(
        ...
        'deadbandは発火と両立せず断念。正相関=小さい正リターン後のボラ上昇なので'
        '上昇側チャネルを疑う。\n'
    ...
    aggdf.to_csv(ROOT / 'comparison_round13.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round13.csv', index=False)
