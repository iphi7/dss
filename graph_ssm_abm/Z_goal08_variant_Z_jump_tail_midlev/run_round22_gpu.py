"""
Z_goal08 Round22 (最終微調整): abs_acf1 を 0.30 以下へ

Round21 の結果:
  - Z86 (score 0.287): abs1=0.321、kurt=21.3、dec9=-0.120、dec10=-0.204/-0.202 と
    ほぼ全て許容圏だが abs1 が 0.30 をわずかに超える
  - Z85: abs1=0.309

最後の一押し (iid成分をさらに少し増やす):
  Z87: Z86 + participation_noise 0.35→0.50
  Z88: Z86 + exog_common_sigma 0.0058→0.0064, pi 0.021→0.019
  Z89: 両方
"""
...
RESULTS = ROOT / 'results_gpu_round22'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
CONFIGS = {
    'Z87_pnoise05': z86_base(participation_noise_sigma=0.50),
    'Z88_moreiid': z86_base(exog_common_sigma=0.0064, price_impact=0.019),
    'Z89_both': z86_base(
        participation_noise_sigma=0.50,
...
def main():
    ...
    aggdf.to_csv(ROOT / 'comparison_round22.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round22.csv', index=False)
