"""
Z_goal08 Round14 (構造修正の統合): 低分位正相関の修正を確定する

Round13 の切り分け結果:
  - 低分位 (dec3) 正相関の主犯は市場アンカー (Z53で dec3 +0.00→-0.02) と
    観測 momentum (Z52で -0.01)。fear/退避は無関係
  - 機序: 小さい正リターン → 価格がアンカー上へ → 引き戻し圧 + momentum観測強気
    → 取引・変動増、という上昇側チャネル
  - ただしアンカー完全除去は年率リターンが負に流れる (seed3: -4.8%/年) ため不可

Round14 は momentum除去 + アンカー弱体化の統合:
  Z56: Z50 + obs_momentum_scale=0 + anchor 0.006→0.002
  Z57: Z56 の anchor さらに弱く 0.001
  Z58: momentum除去 + anchor強度は保ちgap_scaleを広げる別形 (0.70→1.60)
"""
...
RESULTS = ROOT / 'results_gpu_round14'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
CONFIGS = {
    'Z56_nomom_anchor002': z50_base(
        obs_momentum_scale=0.0, market_anchor_strength=0.002,
    ),
    'Z57_nomom_anchor001': z50_base(
        obs_momentum_scale=0.0, market_anchor_strength=0.001,
    ),
    'Z58_nomom_widegap': z50_base(
        obs_momentum_scale=0.0, market_anchor_gap_scale=1.60,
    ),
}
...
def main():
    ...
    append(
        ...
        'Round13で低分位正相関の主犯=市場アンカー+観測momentumと特定。'
        'アンカー完全除去は長期ドリフトが負に流れるため弱体化で統合。\n'
    ...
    aggdf.to_csv(ROOT / 'comparison_round14.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round14.csv', index=False)
