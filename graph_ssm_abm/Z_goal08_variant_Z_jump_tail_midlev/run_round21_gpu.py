"""
Z_goal08 Round21: abs削減の統合仕上げ

Round20 の結果:
  - Z81 (iidノイズ再配分: exog_sigma 0.00445→0.0058, pi 0.025→0.021) が決定打:
    abs1/2/3/5 = 0.310/0.288/0.272/0.243 (実 0.261/0.296/0.280/0.289) とほぼ実形状
  - Z80 (参加率日次ノイズ 0.35) も -0.04 の追加削減。併用可能
  - Z81 の副作用: mid_5d -0.076→-0.036、dec9 -0.122→-0.077 (取引成分比率の低下)、
    kurt=36.8 (stdが下がり危機日が相対的に大きく)

  Z83: Z81 + pnoise0.35 (abs1 を 0.28 前後へ)
  Z84: Z83 + asym_pi_scale 1.0→1.3 (mid/dec9 の回復)
  Z85: Z84 + disaster_sigma 0.048→0.042 (kurt 制御)
  Z86: Z84 + disaster_sigma 0.044 + jump2_sigma 0.020→0.016 (kurt+q999 制御)
"""
...
RESULTS = ROOT / 'results_gpu_round21'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
CONFIGS = {
    'Z83_shift_pnoise': z83_base(),
    'Z84_asym13': z83_base(asym_pi_scale=1.3),
    'Z85_asym13_ds042': z83_base(asym_pi_scale=1.3, disaster_sigma=0.042),
    'Z86_asym13_thin': z83_base(
...
def main():
    ...
    append(
        '## Round21: abs削減の統合仕上げ\n\n'
        'Z81(iid再配分)でabs形状ほぼ実データに。副作用のmid/dec9弱化とkurtを'
        'pnoise併用+asym増強+危機σ微減で補正。\n'
    ...
    aggdf.to_csv(ROOT / 'comparison_round21.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round21.csv', index=False)
