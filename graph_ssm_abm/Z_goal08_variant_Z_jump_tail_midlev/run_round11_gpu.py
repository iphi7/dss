"""
Z_goal08 Round11: 構造的修正 (デッドバンド + 災害エピソード)

Z43 への指摘と構造的対応:
  1. 分位3-4 の正相関 (全モデル共通の未解決アーティファクト)
     → down_deadband: 閾値未満の微小下落は down_var/ストレスに蓄積しない。
       「微小下落だけが記憶される非対称」が低分位の正相関源という仮説。
  2. 超特大ジャンプ過多 (>10%が60年で12日 vs 実データ3日) + sq_acf弱
     → 災害エピソード: 単日特大ジャンプを廃し、数日continueする危機に置き換え。
       実データの2008/2020は-5〜-9%日の連続で、単日-20%は1987の1回のみ。
       連続する大|r|がr^2クラスタを作りsq_acfの構造的な源になる。
  3. abs_acf過剰 (全モデル共通) → ストレス記憶の最長を短縮する腕も検証。

  Z44: Z43 + deadband=0.005 (①の分離検証)
  Z45: Z43 - 特大ジャンプ + エピソード(p=0.0006, σ=0.045, decay=0.62) (②③の分離検証)
  Z46: 両方
  Z47: Z46 + stress decay_max 0.975 (abs_acf20抑制)
"""
...
RESULTS = ROOT / 'results_gpu_round11'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
def z43_base(**overrides):
    params = dict(
        ...
        exog_common_jump_prob=0.0006, exog_common_jump_sigma=0.13, jump_df=2.5,
...
CONFIGS = {
    'Z44_deadband': z43_base(down_deadband=0.005),
    'Z45_episode': z43_base(
    ...
    'Z47_both_calm': z43_base(
        down_deadband=0.005,
        exog_common_jump_prob=0.0,
        disaster_prob=0.0006, disaster_sigma=0.045,
        disaster_df=4.0, disaster_decay=0.62, disaster_end=0.10,
        investor_stress_decay_max=0.975,
    ),
}
...
def main():
    ...
    append(
        ...
        '①低分位正相関→down_deadband(微小下落を記憶しない)、'
        '②③特大過多+sq_acf弱→災害エピソード(単日特大を複数日クラッシュに分割)。\n'
    ...
    aggdf.to_csv(ROOT / 'comparison_round11.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round11.csv', index=False)
