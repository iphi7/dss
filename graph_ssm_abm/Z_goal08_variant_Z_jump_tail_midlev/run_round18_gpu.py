"""
Z_goal08 Round18: エピソードのプラトー化 (sq_acf lag5-10 の谷を埋める)

Round17 の結果:
  - sq1/2/3 は実データ水準に到達 (Z67: 0.253/0.231/0.216 vs 実0.230/0.273/0.180)
  - 残り: sq5/10 の谷 (0.127/0.081 vs 実0.228/0.136)。幾何減衰が lag5 で切れる形
  - q999 が微太り (0.096 vs 0.070)、abs1 過剰 (0.41-0.45 vs 0.26) は変わらず

対応: disaster_plateau — 危機強度を数日間 1.0 に維持してから減衰。
     実データの sq が lag5 まで平坦 = 危機ボラの1〜2週間の高止まりに対応。
     q999 制御のため σ をやや下げる。

  Z71: Z68 + plateau=6, decay0.78, σ0.065
  Z72: Z71 + plateau=10 (より長い高止まり)
  Z73: Z71 + σ0.058, 余震45 (q999 を 0.070 へ)
  Z74: Z72 + participation_vol_power 1.21→1.00 (abs1 過剰の削減試行)
"""
...
RESULTS = ROOT / 'results_gpu_round18'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
CONFIGS = {
    'Z71_plateau6': z71_base(),
    'Z72_plateau10': z71_base(disaster_plateau=10),
    'Z73_plateau6_thin': z71_base(disaster_sigma=0.058, jump_aftershock_scale=45.0),
    'Z74_plateau10_lowpart': z71_base(disaster_plateau=10, participation_vol_power=1.00),
}
...
def main():
    ...
    append(
        '## Round18: エピソードのプラトー化\n\n'
        'sq1-3は実データ水準到達。sq5/10の谷はエピソード幾何減衰がlag5で切れるため。'
        '危機強度を数日維持するプラトーを導入 (実データの危機ボラ高止まりに対応)。\n'
    ...
    aggdf.to_csv(ROOT / 'comparison_round18.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round18.csv', index=False)
