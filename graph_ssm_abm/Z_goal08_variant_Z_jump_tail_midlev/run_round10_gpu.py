"""
Z_goal08 Round10 (最終): Z38 の leverage 強化と tail 微調整

Round9 の結果 (Z38, score 0.356 最良):
  - 分位9問題解消: 1d=-0.094 (実-0.089)。分位プロファイルが単調な形に
  - std=0.0115/kurt=22.8/abs_acf5=0.282 とコア一致
  - 残り: leverage全体が約0.05弱い、q999/max やや細い、sq_acf=0.078 (実0.228)

Round10:
  Z41: Z38 + asym_pi_scale=1.0 (leverage全体を強化)
  Z42: Z41 + 特大ジャンプ増強 (p=0.0006, σ=0.13) — q999/max
  Z43: Z42 + 余震32/0.72 — sq_acf
"""
...
RESULTS = ROOT / 'results_gpu_round10'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
CONFIGS = {
    'Z41_asym10': z38_base(asym_pi_scale=1.0),
    'Z42_asym10_tail': z38_base(
        asym_pi_scale=1.0,
        exog_common_jump_prob=0.0006, exog_common_jump_sigma=0.13,
    ),
    'Z43_full': z38_base(
        asym_pi_scale=1.0,
        exog_common_jump_prob=0.0006, exog_common_jump_sigma=0.13,
...
def main():
    ...
    append(
        ...
        'Z38で分位9問題解消(1d=-0.094 vs 実-0.089)。残り: leverage全体約0.05弱、'
        'q999/max細い、sq_acf途上。\n'
    ...
    aggdf.to_csv(ROOT / 'comparison_round10.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round10.csv', index=False)
