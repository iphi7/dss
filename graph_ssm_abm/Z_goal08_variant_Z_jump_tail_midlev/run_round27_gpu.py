"""
Z_goal08 Round27: DGS10 の内生生成 (相互作用モード)

ユーザー要件: DGS10 を実データ入力でなく、1966年初期値から株価と相互作用しながら
逐次生成する。
  金利→株: 投資家スコア (水準ペナルティ + kappa×前日変化のレジーム項)
  株→金利: 当日株リターン×kappa を金利変化に加算 (質への逃避/リスクオン連動)

スモークテスト (seed2): 水準の大波・persistence・相関レジーム両側の出現を確認。
調整点: 変化std 0.106 (実0.067)、kurt 5.5 (実8.4)、|Δ|acf1 0.107 (実0.254)、上限張り付き。

評価変更: 実データとの日付整列がなくなるため era 指標は score から外し、
ローリング相関の分布 (q05/q50/q95/正割合) と DGS10 自体の統計で評価する。

  Z104: 校正ベース (σ0=0.036, df=3.5, λ=0.97)
  Z105: + drift_sigma 0.00006 (大波を穏やかに、張り付き回避)
  Z106: + stock_beta 3.0 (相関の振れ幅拡大)
  Z107: + mr_theta 0.00006 (中心回帰強め、張り付き回避の別形)
"""
...
RESULTS = ROOT / 'results_gpu_round27'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
CONFIGS = {
    'Z104_calib': z104_base(),
    'Z105_softdrift': z104_base(dgs10_drift_sigma=0.00006),
    'Z106_beta30': z104_base(dgs10_stock_beta=3.0),
    'Z107_mr6': z104_base(dgs10_mr_theta=0.00006),
}
...
def main():
    ...
    tgt['rc_rmse'] = 0.0
    append(
        '## Round27: DGS10内生生成 (相互作用モード)\n\n'
        '金利→株 (スコア) と株→金利 (当日リターン×kappa) の双方向逐次生成。'
        '評価はローリング相関の分布 + DGS10自体の統計へ変更。\n'
        'Z104=校正、Z105=drift弱め、Z106=結合強め、Z107=回帰強め。'
    )
    all_rows = []
    aggs = []
    ...
    aggdf.to_csv(ROOT / 'comparison_round27.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round27.csv', index=False)
