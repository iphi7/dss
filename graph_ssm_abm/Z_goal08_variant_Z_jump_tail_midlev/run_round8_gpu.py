"""
Z_goal08 Round8: ユーザーフィードバック対応 (sq_acf / 分位9 / 2層ジャンプ)

指摘と診断:
  1. sq_acf が不自然に低い (0.05-0.08 vs 実 0.23)。原因: クリップ解放後の巨大ジャンプが
     iid で入り、r^2 の分散を単発外れ値が支配して相関を薄める。
     (R03/Y14 では firm_return_clip=0.08 がジャンプを切っていたため sq_acf が保たれていた)
  2. 分位9 leverage: 1d=-0.165 (実-0.089) と過剰、10d/20d では逆に弱い。
     反応が高分位に集中しすぎ + 短命すぎる。
  3. 実データの Q-Q は「中規模ジャンプがそこそこ + 特大ジャンプがごく稀」の2層構造。

対応:
  - 2層ジャンプ: 第1層=特大・超レア (p≈0.0004, σ≈0.15)、第2層=中規模・年2回 (p≈0.008, σ≈0.035)
  - 第2層×余震で r^2 クラスタを作り sq_acf を回復
  - asym_pi 再形状化: thr 0.55→0.25、sat 1.3→0.8、down_decay 0.60→0.75
    (分位5-8を持ち上げ、分位9の1d過剰を圧縮、7d以降を持続)

  Z33: 2層ジャンプのみ (asym は Z32 のまま)
  Z34: Z33 + asym 再形状化
  Z35: Z34 + 余震強化 (35, 0.75)
  Z36: Z34 + 第2層増強 (p=0.012, σ=0.030)
"""
...
RESULTS = ROOT / 'results_gpu_round8'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
def z33_base(**overrides):
    """Z32 から 2層ジャンプへ置き換えたベース。"""
    params = dict(
        investor_stress_scale=1.4,
        investor_stress_decay_min=0.90, investor_stress_decay_max=0.988,
        vol_sensitivity_mean=0.65, jump_vol_coupling=0.5,
        # 第1層: 特大・超レア (10年に1回程度)
        exog_common_jump_prob=0.0004, exog_common_jump_sigma=0.15, jump_df=2.5,
        # 第2層: 中規模・年2回程度
        exog_common_jump2_prob=0.008, exog_common_jump2_sigma=0.035, jump2_df=5.0,
        # 余震: 中規模ジャンプでもクラスタを作る
        jump_aftershock_scale=25.0, jump_aftershock_decay=0.70,
    )
    params.update(overrides)
...
CONFIGS = {
    'Z33_twotier': z33_base(),
    'Z34_twotier_asym': z33_base(
        asym_pi_scale=0.80, asym_pi_threshold=0.25, asym_pi_sat=0.8,
        down_ewma_decay=0.75,
    ),
    'Z35_z34_aftershock': z33_base(
        asym_pi_scale=0.80, asym_pi_threshold=0.25, asym_pi_sat=0.8,
        down_ewma_decay=0.75,
        jump_aftershock_scale=35.0, jump_aftershock_decay=0.75,
    ),
    'Z36_z34_jump2plus': z33_base(
        asym_pi_scale=0.80, asym_pi_threshold=0.25, asym_pi_sat=0.8,
        down_ewma_decay=0.75,
        exog_common_jump2_prob=0.012, exog_common_jump2_sigma=0.030,
    ),
}
...
def main():
    ...
    append(
        ...
        '指摘: sq_acf低すぎ(iidジャンプがr2を支配)、分位9の1d過剰(-0.165 vs -0.089)+短命、'
        'ジャンプは中規模そこそこ+特大ごく稀の2層構造が実データに近い。\n'
        'Z33=2層ジャンプ、Z34=+asym再形状(thr0.25/sat0.8/decay0.75)、'
        'Z35=+余震強化、Z36=+第2層増強。'
    )
    all_rows = []
    aggs = []
    ...
    aggdf.to_csv(ROOT / 'comparison_round8.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round8.csv', index=False)
