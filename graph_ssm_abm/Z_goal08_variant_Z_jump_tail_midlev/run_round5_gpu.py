"""
Z_goal08 Round5: Z18 (層化抽出) を軸に発火信頼性・20d leverage・tail の仕上げ

Round4 の結果:
  - Z18_strat が過去最良 (score 0.396)。seed3 も発火するようになった
  - 残る課題: seed1 の部分発火 (std 0.0084)、dec10 20d 弱い (-0.04 vs -0.16)、
    q999 不足 (0.053 vs 0.070)
  - corr増強(Z19)や n_inv=100(Z20) は逆効果 → Z18 の形を維持

Round5:
  Z22: Z18 + vol_sensitivity_mean=0.65 (リスク選好の総量を増やし発火を確実に)
  Z23: Z18 + wealth_sigma=1.15 (クジラを大きく)
  Z24: Z18 + investor_stress_scale=1.4 (ストレス発火を強く)
  Z25: Z18 + 投資家別ストレス記憶 (0.90-0.988) — 20d leverage
  Z26: Z18 + jump_sigma=0.12/prob=0.0018 — q999
"""
...
CONFIGS = {
    'Z22_vsmean065': z18_base(vol_sensitivity_mean=0.65),
    'Z23_wsigma115': z18_base(wealth_sigma=1.15),
    'Z24_stress14': z18_base(investor_stress_scale=1.4),
    'Z25_hetero': z18_base(
        investor_stress_decay_min=0.90, investor_stress_decay_max=0.988,
    ),
    'Z26_tail': z18_base(
        exog_common_jump_prob=0.0018, exog_common_jump_sigma=0.12,
    ),
}
