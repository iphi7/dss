"""
Z_goal08 Round29: 生成DGS10の最終校正

Round28 の結果 (Z111):
  - 金利側ほぼ校正完了: dy_std=0.065 (実0.067)、張り付き解消 (max14.6)、acf250=0.947
  - 株側大幅回復: std=0.012、dec10=-0.242
  - 残り: (a) sq_acf1=0.354 — 金利ボラクラスタがスコア共通項 (κ×Δy) 経由で株に輸入
          (b) 金利尖度14 (実8.4) 高い、|Δ|クラスタ0.12 (実0.25) 弱い
          (c) 相関 q05=-0.61 やや深い (scoreβ低下で同時に是正される見込み)

  Z112: Z111 + score_beta 0.7→0.55 (株sq保護 + q05是正)
  Z113: Z112 + df 3.5→4.5 (金利尖度) + λ 0.98→0.95 (クラスタ応答を速く)
  Z114: Z113 + stock_beta 2.5→2.2 (相関幅微調整)
  Z115: Z113 + score_beta 0.45 (さらに保護)
"""
...
RESULTS = ROOT / 'results_gpu_round29'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
CONFIGS = {
    'Z112_sb055': z111_base(rate_change_score_beta=0.55),
    'Z113_df45_l95': z111_base(
        rate_change_score_beta=0.55, dgs10_df=4.5, dgs10_vol_lambda=0.95,
    ),
    'Z114_stockb22': z111_base(
        rate_change_score_beta=0.55, dgs10_df=4.5, dgs10_vol_lambda=0.95,
        dgs10_stock_beta=2.2,
    ),
    'Z115_sb045': z111_base(
        rate_change_score_beta=0.45, dgs10_df=4.5, dgs10_vol_lambda=0.95,
    ),
}
...
def main():
    ...
    tgt['rc_rmse'] = 0.0
    append(
        '## Round29: 生成DGS10の最終校正\n\n'
        'Z111で金利側ほぼ完了。残り: 株sq_acf1 (金利クラスタの輸入)、金利尖度/クラスタ。\n'
        'Z112=scoreβ0.55、Z113=+df4.5/λ0.95、Z114=+stockβ2.2、Z115=scoreβ0.45。'
    )
    all_rows = []
    aggs = []
    ...
    aggdf.to_csv(ROOT / 'comparison_round29.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round29.csv', index=False)
