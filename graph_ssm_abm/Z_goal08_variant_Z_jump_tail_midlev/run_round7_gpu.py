"""
Z_goal08 Round7 (最終): Z30 の tail 過剰の微調整

Round6 の結果:
  - Z30 (統合+vc0.5+vs_mean0.65) が score 0.376 と最良。3 seed 全てが発火
  - Z30 seed2: kurt=24.4 (目標21.8)、abs_acf5=0.286 (目標0.289) とほぼ一致
  - 課題: ボラ連動がジャンプを増幅し q999=0.094 (目標0.070) と過剰

Round7 は jump_sigma を下げて tail を目標に合わせる:
  Z31: Z30 + jump_sigma=0.095
  Z32: Z30 + jump_sigma=0.085
"""
...
RESULTS = ROOT / 'results_gpu_round7'
MEMO = ROOT / '検証メモ.md'
SEEDS = [1, 2, 3]
...
def z30_base(**overrides):
    params = dict(
        ...
        jump_vol_coupling=0.5, vol_sensitivity_mean=0.65,
    )
    params.update(overrides)
...
CONFIGS = {
    'Z31_jsig095': z30_base(exog_common_jump_sigma=0.095),
    'Z32_jsig085': z30_base(exog_common_jump_sigma=0.085),
}
...
def main():
    ...
    aggdf.to_csv(ROOT / 'comparison_round7.csv', index=False)
    fulldf.to_csv(ROOT / 'full_metrics_round7.csv', index=False)
