...
CONFIGS={
    # R03: ACF/tailはよいが、短期decile10が強く20dが弱い。まず弱いrebalを混ぜる。
    'X01_R03_tiny_rebal': cfg(
        exog_common_jump_prob=0.006, exog_common_jump_sigma=0.040,
        rare_shock_prob=0.020, rare_shock_sigma=0.13,
        portfolio_rebalance_rate=0.0005,
    ),
    # R03の即時leverageを落として、activityでACFを残す。
    'X02_R03_less_fear_activity': cfg(
        exog_common_jump_prob=0.006, exog_common_jump_sigma=0.040,
        rare_shock_prob=0.020, rare_shock_sigma=0.13,
        stoploss_universal_scale=0.006, asym_pi_scale=0.70, down_ewma_decay=0.50,
        participation_vol_power=1.10, impact_activity_scale=1.10,
    ),
    # R12: decile形状はR内で近いがACF弱め。rebalをさらに弱め、tail/ACFを戻す。
    'X03_R12_half_rebal_more_acf': cfg(
        ...
        participation_vol_power=1.35, impact_activity_scale=1.75, impact_activity_clip=3.40,
        asym_pi_scale=1.15, down_ewma_decay=0.60,
        stoploss_universal_scale=0.018, stoploss_universal_threshold=0.004,
        portfolio_rebalance_rate=0.0005,
    ),
    # R12の即時fearを弱くし、asym_piを少し戻す。短期dec10過剰抑制狙い。
    'X04_R12_low_fear_mid_asym': cfg(
        idio_vol=0.0080, exog_common_sigma=0.0045,
        exog_common_jump_prob=0.008, exog_common_jump_sigma=0.040,
        rare_shock_prob=0.022, rare_shock_sigma=0.14,
        participation_vol_power=1.30, impact_activity_scale=1.60, impact_activity_clip=3.20,
        asym_pi_scale=0.95, down_ewma_decay=0.62,
        stoploss_universal_scale=0.008, stoploss_universal_threshold=0.004,
        portfolio_rebalance_rate=0.0008,
    ),
    # R10/R11系: decile shapeはそこそこだがACFが弱い。activity/tailだけ戻す。
    'X05_rebal_shape_tail_restore': cfg(
        idio_vol=0.0076, exog_common_sigma=0.0042,
        exog_common_jump_prob=0.008, exog_common_jump_sigma=0.040,
        rare_shock_prob=0.025, rare_shock_sigma=0.15,
        participation_vol_power=1.15, impact_activity_scale=1.25, impact_activity_clip=2.80,
        asym_pi_scale=0.90, down_ewma_decay=0.58,
        stoploss_universal_scale=0.010, stoploss_universal_threshold=0.004,
        portfolio_rebalance_rate=0.003,
    ),
    # 尖度寄り: rare shockを増やすがfearを弱め、QQの曲がりすぎを避ける軽量案。
    'X06_tail_light_low_fear': cfg(
        idio_vol=0.0070, exog_common_sigma=0.0038,
        exog_common_jump_prob=0.006, exog_common_jump_sigma=0.038,
        rare_shock_prob=0.035, rare_shock_sigma=0.15,
        participation_vol_power=1.00, impact_activity_scale=0.95, impact_activity_clip=2.40,
        asym_pi_scale=0.75, down_ewma_decay=0.55,
        stoploss_universal_scale=0.005, stoploss_universal_threshold=0.005,
        portfolio_rebalance_rate=0.0005,
    ),
}
