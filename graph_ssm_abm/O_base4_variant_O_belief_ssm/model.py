...
class Config:
    ...
    # 流動性クラッシュ: 取引量fast/slow比率が閾値を超えたときだけ、price impactを非線形に増幅する
    impact_crash_threshold: float = 1.50
    impact_crash_scale:     float = 0.0
    impact_crash_power:     float = 2.0
    ...
    # Kalmanなし belief-state SSM の固定更新率
...
def simulate_market(
    ...
    # 主観グラフの重み精度スコア。private anchor 更新率に使う。
    ...
    # 投資家ごとの固定された観測更新癖。大きいほど新情報を信じやすい。
    ...
    # 各投資家が持つ内部推定状態: belief_state[i, firm, dim]
    # Kalman filterではなく、予測 + 固定alphaの観測ブレンドで更新する。
    ...
    for t in range(t_max):
        ...
        # private次元ごとの anchor 観測を作る。
        # public/sector観測の固定対応表から、直接観測できないprivate状態の手がかりを作る。
        y_obs_global = np.concatenate([y_pub, y_sec], axis=1)  # dims 0..13
        anchor_specs = [
            [(0, 0.60), (3, 0.40)],                    # priv 14: 財務/負債系
            [(1, 0.50), (6, 0.50)],                    # priv 15: 収益性 + sector0
            [(8, 0.50), (9, 0.50)],                    # priv 16: supply/sector link
            [(4, 0.50), (10, 0.50)],                   # priv 17: 成長/需要
            [(2, 0.50), (3, 0.50)],                    # priv 18: 流動性/信用
            [(11, 0.34), (12, 0.33), (13, 0.33)],      # priv 19: 業界ショック
        ]
        ...
        # --- 投資家ループ: Kalmanなし belief-state SSM ---
        for i, p in enumerate(params):
            ...
            # 2. public dims: 全員観測できるが、非専門企業は少し更新率を下げる
            ...
            # 3. sector dims: 専門家は強く、非専門家はほぼ更新できない
            ...
            # 4. private dims: 直接観測せず、anchorを主観グラフで集約した擬似観測で更新
            if config.use_graph:
                alpha_priv = (
        ...
        impact_factor_raw = 1.0 + config.impact_activity_scale * max(0.0, volume_ratio - 1.0)
