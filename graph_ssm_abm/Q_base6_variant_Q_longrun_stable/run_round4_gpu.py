...
def cfg(**overrides):
    params=dict(
        ...
        impact_activity_scale=1.50,
        impact_activity_clip=3.50,
...
CONFIGS={
 'Q13_lowimpact_fear02': cfg(stoploss_universal_scale=0.02, stoploss_universal_threshold=0.004),
 'Q14_lowimpact_asym3': cfg(asym_pi_scale=3.00, down_ewma_decay=0.86),
 'Q15_lowimpact_fear03_asym25': cfg(stoploss_universal_scale=0.03, stoploss_universal_threshold=0.004, asym_pi_scale=2.50, down_ewma_decay=0.86),
 'Q16_lowimpact_moreacf': cfg(stoploss_universal_scale=0.02, stoploss_universal_threshold=0.004, asym_pi_scale=2.50, down_ewma_decay=0.86, impact_activity_scale=2.00, impact_activity_clip=4.00),
}
...
def main():
    ...
    append('## Round4: Q12ベースで leverage/ACF を戻す\n\nRound3でQ12が60年安定・尖度良好だったため、Q12を土台に弱い market-wide fear と非対称 price impact を再導入。目的は崩壊させずに leverage を負方向へ、abs ACF を実データ近傍へ戻すこと。')
    all_roll=[]; all_full=[]; all_agg=[]
    for label,c0 in CONFIGS.items():
        t0=time.time(); rolls=[]; fulls=[]
        append(f'### {label}\n\nparams: stoploss_universal={c0.stoploss_universal_scale}, asym_pi={c0.asym_pi_scale}, down_decay={c0.down_ewma_decay}, impact_activity_scale={c0.impact_activity_scale}, clip={c0.impact_activity_clip}')
        for seed in SEEDS:
    ...
    aggdf.to_csv(ROOT/'comparison_longrun_round4.csv',index=False)
    pd.DataFrame(all_roll).to_csv(ROOT/'rolling_metrics_round4.csv',index=False)
    pd.DataFrame(all_full).to_csv(ROOT/'full_metrics_round4.csv',index=False)
