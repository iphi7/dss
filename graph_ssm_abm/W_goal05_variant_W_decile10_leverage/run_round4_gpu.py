...
CONFIGS={
    # Round4: W04/W07/W10の弱点だった「短期は強すぎ、10d/20dは弱すぎ」を、
    # 下落後に数日遅れて企業固有ノイズだけを増幅する delayed_vol で補正する。
    'W11_W10_delay_soft': cfg(
        stoploss_universal_scale=0.0, asym_pi_scale=0.20, down_ewma_decay=0.62,
        participation_vol_power=0.86, impact_activity_scale=0.78,
        delayed_vol_scale=0.55, delayed_vol_decay=0.93, delayed_vol_delay=3, delayed_vol_clip=1.25,
    ),
    'W12_W10_delay_mid': cfg(
        stoploss_universal_scale=0.0, asym_pi_scale=0.18, down_ewma_decay=0.62,
        participation_vol_power=0.86, impact_activity_scale=0.78,
        delayed_vol_scale=0.85, delayed_vol_decay=0.94, delayed_vol_delay=4, delayed_vol_clip=1.35,
    ),
    'W13_W07_delay_mid': cfg(
        stoploss_universal_scale=0.0, asym_pi_scale=0.20, down_ewma_decay=0.60,
        participation_vol_power=0.76, impact_activity_scale=0.62,
        delayed_vol_scale=0.90, delayed_vol_decay=0.94, delayed_vol_delay=3, delayed_vol_clip=1.35,
    ),
    'W14_W09_delay_soft': cfg(
        stoploss_universal_scale=0.0, asym_pi_scale=0.23, down_ewma_decay=0.55,
        participation_vol_power=0.78, impact_activity_scale=0.66,
        delayed_vol_scale=0.60, delayed_vol_decay=0.93, delayed_vol_delay=3, delayed_vol_clip=1.30,
    ),
    'W15_W04_delay_soft': cfg(
        stoploss_universal_scale=0.0, asym_pi_scale=0.35, down_ewma_decay=0.50,
        participation_vol_power=0.82, impact_activity_scale=0.58,
        delayed_vol_scale=0.50, delayed_vol_decay=0.92, delayed_vol_delay=4, delayed_vol_clip=1.25,
    ),
}
