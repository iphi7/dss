...
CONFIGS={
    # W04 issue: 1d/3d too negative, 10d/20d too weak. Try weaker but slower down-memory.
    'W07_slow_weak_asym20': cfg(stoploss_universal_scale=0.0, asym_pi_scale=0.20, down_ewma_decay=0.60, participation_vol_power=0.76, impact_activity_scale=0.62),
    'W08_slower_asym15': cfg(stoploss_universal_scale=0.0, asym_pi_scale=0.15, down_ewma_decay=0.72, participation_vol_power=0.76, impact_activity_scale=0.62),
    'W09_mid_slow_asym25': cfg(stoploss_universal_scale=0.0, asym_pi_scale=0.25, down_ewma_decay=0.55, participation_vol_power=0.78, impact_activity_scale=0.66),
    'W10_asym20_more_activity': cfg(stoploss_universal_scale=0.0, asym_pi_scale=0.20, down_ewma_decay=0.62, participation_vol_power=0.86, impact_activity_scale=0.78),
}
...
def main():
    ...
    MEMO.write_text('# W_goal05_variant_W_decile10_leverage 検証メモ\n\n',encoding='utf-8')
    append('## Round3: 弱く長いdown-memoryでdecile10曲線をなだらかにする\n\nW04では5dは合ったが、1d/3dが強く10d/20dが弱い。即時非対称性をさらに弱め、down-memoryを長めにして曲線をなだらかにする。')
    all_rows=[]; aggs=[]
