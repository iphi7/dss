...
def simulate_market_gpu(
    ...
    prev_sp_ret = 0.0
    delayed_vol_state = 0.0
    delayed_vol_queue = [0.0 for _ in range(max(0, int(config.delayed_vol_delay)))]
    ...
    for t in range(t_max):
        ...
        if config.delayed_vol_scale > 0.0 and delayed_vol_state > 0.0:
            # 下落直後ではなく数日遅れて企業固有ノイズを増幅する。
            # decile10 の 1d/3d レバレッジを過度に強めず、
            # 10d/20d 側の将来ボラだけを少し戻すための機構。
            delayed_mult = float(np.sqrt(
                1.0 + config.delayed_vol_scale * delayed_vol_state / (config.market_vol ** 2 + 1e-12)
            ))
            delayed_mult = float(np.clip(delayed_mult, 1.0, config.delayed_vol_clip))
            noise = noise * delayed_mult
        ...
        firm_return = config.price_impact * impact_factor * imbalance + common_noise + noise + fundamental_return + market_anchor_return
        ...
        if config.delayed_vol_scale > 0.0:
            incoming = neg_ret ** 2
            if delayed_vol_queue:
                matured = delayed_vol_queue.pop(0)
                delayed_vol_queue.append(incoming)
            else:
                matured = incoming
            delayed_vol_state = (
                config.delayed_vol_decay * delayed_vol_state
                + (1.0 - config.delayed_vol_decay) * matured
            )
