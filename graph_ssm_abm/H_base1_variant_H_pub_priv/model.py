...
class Config:
    ...
    obs_sigma: float = 0.045          # y = s + noise

    # 非公開成分 h の真の動態
    # 安定条件: phi_h + rho_h < 1 (行正規化Wの最大固有値=1 のため)
    # 旧値: phi_h=0.90, rho_h=0.35 → 1.25 > 1 で発散バグあり
    phi_h: float = 0.65
    rho_h: float = 0.30               # 0.65 + 0.30 = 0.95 < 1 → 安定
    ...
    # アブレーションフラグ
    use_graph: bool = True   # Falseにすると投資家がW_iを無視（h推定不能）
...
def make_subjective_graphs(
    ...
    for i in range(n_investors):
        ...
        # 投資家が信じる h の伝播係数（W_i @ y で h を推定する強さ）
...
def simulate_market(
    ...
    for t in range(t_max):
        ...
        # 観測: s のみ（ノイズ付き）
        y = s + 0.35 * (s - s_prev) + rng.normal(0.0, config.obs_sigma, size=n)

        buy_value = np.zeros(n, dtype=float)
        sell_value = np.zeros(n, dtype=float)
        agg_total_est = np.zeros(n, dtype=float)  # 市場集計推定値（market_stress用）

        momentum = np.zeros(n) if t == 0 else firm_return

        for i, p in enumerate(params):
            if config.use_graph:
                # グラフあり: 隣接企業の観測から h を推定
                est_h = p["belief_rho_h"] * (subjective_graphs[i] @ y)
                total_est = y + est_h
                # 来期予測（s と h の合計を予測）
                pred_next = p["belief_phi"] * total_est + p["belief_rho_s"] * (subjective_graphs[i] @ total_est)
            else:
                # グラフなし: 観測 y のみ、h の推定不能
                total_est = y
                pred_next = p["belief_phi"] * y

            agg_total_est += total_est  # 各投資家の推定値を集計
        ...
        # --- 価格形成: 注文不均衡のみ（真の x を直接介入させない）---
        # market_stress: 投資家の集計推定値の絶対値平均を使う
        # 真のxの代わりに市場のコンセンサス推定（y ≈ s ベースで安定）を使うことで
        # sの自己相関（phi_s=0.82）がGARCHのベース分散を持続的に駆動する
