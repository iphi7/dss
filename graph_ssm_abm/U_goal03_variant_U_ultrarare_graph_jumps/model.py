...
class Config:
    ...
    # ===== グラフベースイベントジャンプ (T系) =====
    # 局所企業ショックを真の企業ネットワーク上に伝播させる。
    graph_jump_prob: float = 0.0
    graph_jump_sigma: float = 0.0
    graph_jump_df: float = 2.8
    graph_jump_clip: float = 0.18
    graph_jump_neg_prob: float = 0.60
    graph_jump_sources: int = 2
    graph_jump_degree_power: float = 1.0
    graph_jump_spread: float = 0.55
    graph_jump_hops: int = 2
    graph_jump_mode: str = "signed"  # signed / abs_noise / residual
    graph_jump_residual_decay: float = 0.0
    graph_jump_residual_scale: float = 0.0
    graph_jump_normalize: str = "mean1"  # mean1 / max1
